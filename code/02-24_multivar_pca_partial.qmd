...
Y is normally referred to as:  
  - Response variable  
  - Dependent variable  
  - Predicted variable  (more to machine learning topics)

X is normally referred to as:  
  - Explanatory variable  
  - Independent variable  
  - Predictor variable  (more to machine learning topics)
...
- Based on Y (outcome) existence:
    - Supervised  (you have the y variable and give it to algorithm so then it can learn with it)
    - Unsupervised (if you ask the algorithm to learn by itself)  
...
It may show you a relationship that may be not true.      
...
lm_dayl.abr <- lm(strength_gtex ~ mean_dayl.s_abr, # This is how we create a simple linear model
                  data = weather) %>%
  tidy() %>%
...
lm_prp.out <- lm(strength_gtex ~ sum_prcp.mm_out,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
lm_dayl.prcp <- lm(strength_gtex ~ mean_dayl.s_abr + 
                   sum_prcp.mm_out,
   data = weather) %>%
  tidy() %>%
  filter(term != "(Intercept)") %>%
...
lm_daylapr.daylmay <- lm(strength_gtex ~ mean_dayl.s_abr + 
                  mean_dayl.s_mai,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
```{r vif uncorrelated}
# Uncorrelated
vif(lm(strength_gtex ~ mean_dayl.s_abr + 
                   sum_prcp.mm_out,
   data = weather))
...
```{r vif correlated}
# Correlated
lm(strength_gtex ~ mean_dayl.s_abr + 
                  mean_dayl.s_mai,
   data = weather) %>%
  vif()
...
weather_n <- weather %>%
  select(-year, -site, -strength_gtex)
...
summary(mod_pca) #Always PC1 will be the most important to explain the variable of the data.
# The goal is to find a subset of PCs that explain most of the variance. 
# Decrease number of features and get features that are not correlated.
```
...
This indicates that original variables were correlated (as we expected). 
  - If they were not all correlate, all PCs would explain a well distributed portion, if some explain way more than others we notice that there is multicolinearity between variables.
