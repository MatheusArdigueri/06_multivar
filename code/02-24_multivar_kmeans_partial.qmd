...
mod_km <- kmeans(weather_norm,
                 centers = 6,
                 nstart = 10 # Nstart makes sure that the algorithm will run 10 times and still return you the best results across the 10 runs
                 )
...
fviz_nbclust(weather_norm,
             method = "wss",
             k.max = 10,
             FUNcluster = kmeans) #choose in the documentation.
```
...
weather %>%
  ...
  group_by(cluster) %>%
  tally() #check the number of observations present in each object.
```
